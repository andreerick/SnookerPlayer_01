...
{
    return m_isPlayer1Turn ? m_player1 : m_player2;
}
...
void Frame::potColor(Ball ball)
{
    if (m_needColor)
    {
        currentPlayer().addPoints(ball.getValue());
...
{
    // IMPORTANT : on vérifie d'abord si une couleur est due
    // (m_needColor), AVANT de vérifier la phase "Reds".
    // Sinon, juste après avoir empoché une rouge (alors qu'il
    // reste encore des rouges sur la table, donc la phase est
    // toujours "Reds"), la fonction réclamait a tort une nouvelle
    // rouge au lieu d'une couleur.
    if (m_needColor)
    {
